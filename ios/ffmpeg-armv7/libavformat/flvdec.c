    ...
    if (key) {
        ...
        if (depth == 1) {
            if (amf_type == AMF_DATA_TYPE_NUMBER ||
                ...
                else if (!strcmp(key, "videodatarate") && (vpar || flv) &&
                         0 <= (int)(num_val * 1024.0)) {
                    if(vpar) vpar->bit_rate = num_val * 1024.0;
                    if(flv) flv->video_bit_rate = num_val * 1024.0;
                } else if (!strcmp(key, "audiodatarate") && (apar || flv) &&
                         0 <= (int)(num_val * 1024.0)) {
                    if(apar) apar->bit_rate = num_val * 1024.0;
                    if(flv) flv->audio_bit_rate = num_val * 1024.0;
