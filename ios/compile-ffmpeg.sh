...
#----------
SDK_COMPILE_SH="tools/do-compile-ffmpeg_sdk.sh"
CCPLAYER_COMPILE_SH="tools/do-compile-ffmpeg_ccplayer.sh"

if [ $FF_TARGET_SDK = "sdk" ] ; then     
    FFMPEG_COMPILE_SH=$SDK_COMPILE_SH
elif [ $FF_TARGET_SDK = "player" ] ; then 
    FFMPEG_COMPILE_SH=$CCPLAYER_COMPILE_SH
else
    echo "Illegal compile mode: "$FF_TARGET_SDK" please give compile mode: sdk or player?"
    exit
fi 

echo "ffmpeg compile shell:"$FFMPEG_COMPILE_SH

if [ "$FF_TARGET" = "armv7" -o "$FF_TARGET" = "armv7s" -o "$FF_TARGET" = "arm64" ]; then
    echo_archs
    sh $FFMPEG_COMPILE_SH $FF_TARGET $FF_TARGET_EXTRA
    do_lipo_all
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        sh $FFMPEG_COMPILE_SH $ARCH $FF_TARGET_EXTRA
    done
