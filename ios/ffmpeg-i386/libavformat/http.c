...
redo:
    ...
    strncpy(h->redirect_url, s->location, AVIO_REDIRECTURL_LEN-1);
    h->redirect_url[AVIO_REDIRECTURL_LEN-1] = 0;
    h->redirect_time = av_gettime_relative() - start_time;
...
static int64_t http_seek_internal(URLContext *h, int64_t off, int whence, int force_reconnect)
{
    ...
    s->hd = NULL;
    av_log(NULL, AV_LOG_INFO,"--- http_seek_internal off %lld ---.\n", off);
...
{
    av_log(NULL,AV_LOG_INFO,"--- http seek off %lld ---.\n", off);
    int64_t ret = -1;
    int retry_times = 2; 
    while((ret = http_seek_internal(h, off, whence, 0)) < 0 && retry_times --) {
        if(ret == AVERROR_HTTP_FORBIDDEN) {
            av_log(NULL, AV_LOG_WARNING,"--- http_seek_internal 403 ---.\n");
        } else {
            break;
        }
    }
